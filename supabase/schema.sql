-- ============================================================================
-- GLENTREE — COMPLETE DATABASE SCHEMA  (single file, run top to bottom)
--
-- WHERE TO RUN IT
--   Supabase Dashboard -> SQL Editor -> New query -> paste -> Run.
--
-- WHAT IT BUILDS
--   The whole Glentree database on an empty Supabase project: tables, the
--   permission helpers, every row-level security policy, the four private
--   storage buckets and their tenant-scoped access rules.
--
-- SAFE TO RE-RUN
--   Every statement is idempotent (CREATE ... IF NOT EXISTS, CREATE OR REPLACE,
--   DROP POLICY IF EXISTS before CREATE POLICY). Running it twice changes
--   nothing the second time.
--
-- CREATES NO USERS AND CONTAINS NO PASSWORDS
--   Staff accounts are made afterwards from the application's own
--   Settings -> Users and roles screen, or with `npm run provision-users`.
--
-- SEARCH PATH IS PINNED ON THE FIRST LINE
--   Supabase's migration runner executes with an empty search_path. Parts of
--   this schema were written unqualified, so without the SET below they fail
--   with "no schema has been selected to create in" -- or, later, with
--   "function app_org() does not exist" while the function is present all
--   along. The SET makes the file run the same way in the SQL Editor, through
--   the CLI, or from a migration runner.
-- ============================================================================

set search_path = public, extensions;


-- ─────────── PART 1 · core platform, bootstrap and messaging ───────────
...
insert into storage.buckets (id, name, public) values
  ...
  -- Chat attachments. Three policies further down reference this bucket, but
  -- nothing created it: on a fresh project every attachment upload failed with
  -- "bucket not found" while the policies looked perfectly correct.
