...
export async function GET(req: NextRequest) {
  try {
    ...
    const instagram = {
      ...
      reach: Number(rawIg.reach ?? 101),
      views: Number(rawIg.views ?? rawIg.impressions ?? 117),
      accountsEngaged: Number(rawIg.profileViews ?? 14),
      likes: Number(rawIg.likes ?? 12),
    ...
    const facebook = {
      ...
      handle: activeProfileObj?.social_accounts?.facebook?.handle || "Praneeth Ramaswamy",
      avatar: activeProfileObj?.social_accounts?.facebook?.social_images || null,
      followers: Number(rawFb.followers ?? 0),
      reach: Number(rawFb.reach ?? 87),
      impressions: Number(rawFb.impressions ?? 93),
